...
CREATE TABLE IF NOT EXISTS core.stock_pb_daily (security_id UINTEGER NOT NULL, trade_date DATE NOT NULL, pb_ratio_lf DOUBLE, pb_ratio_lyr DOUBLE, pb_ratio_ttm DOUBLE, pb_ratio_1_lf DOUBLE, pb_ratio_1_lyr DOUBLE, pb_ratio_1_ttm DOUBLE);
CREATE TABLE IF NOT EXISTS core.stock_market_cap_daily (security_id UINTEGER NOT NULL, trade_date DATE NOT NULL, total_market_cap DOUBLE);
...
CREATE TABLE IF NOT EXISTS meta.stock_adjust_factor_coverage (security_id UINTEGER NOT NULL, start_date DATE NOT NULL, end_date DATE NOT NULL, PRIMARY KEY (security_id, start_date, end_date));
...
CREATE TABLE IF NOT EXISTS meta.stock_pb_daily_coverage (security_id UINTEGER NOT NULL, start_date DATE NOT NULL, end_date DATE NOT NULL, field_set_id UTINYINT NOT NULL, PRIMARY KEY (security_id, start_date, end_date, field_set_id));
CREATE TABLE IF NOT EXISTS meta.stock_market_cap_daily_coverage (security_id UINTEGER NOT NULL, start_date DATE NOT NULL, end_date DATE NOT NULL, field_set_id UTINYINT NOT NULL, PRIMARY KEY (security_id, start_date, end_date, field_set_id));
...
CREATE TABLE IF NOT EXISTS meta.minute_day_status (security_id UINTEGER NOT NULL, trade_date DATE NOT NULL, field_set_id UTINYINT NOT NULL, status UTINYINT NOT NULL, bar_count USMALLINT, return_count USMALLINT, raw_saved BOOLEAN NOT NULL, feature_saved BOOLEAN NOT NULL, updated_at TIMESTAMP NOT NULL, PRIMARY KEY (security_id, trade_date, field_set_id));
CREATE TABLE IF NOT EXISTS meta.minute_download_task (task_id UBIGINT PRIMARY KEY, security_id UINTEGER NOT NULL, start_date DATE NOT NULL, end_date DATE NOT NULL, field_set_id UTINYINT NOT NULL, retain_raw BOOLEAN NOT NULL, status UTINYINT NOT NULL, attempts UTINYINT NOT NULL DEFAULT 0, rows_received UINTEGER, days_received USMALLINT, error_type VARCHAR, error_message VARCHAR, created_at TIMESTAMP NOT NULL, updated_at TIMESTAMP NOT NULL);
...
CREATE OR REPLACE VIEW api.stock_pb_daily AS SELECT s.symbol, d.trade_date AS date, d.pb_ratio_lf, d.pb_ratio_lyr, d.pb_ratio_ttm, d.pb_ratio_1_lf, d.pb_ratio_1_lyr, d.pb_ratio_1_ttm FROM core.stock_pb_daily AS d JOIN ref.security AS s USING (security_id);
CREATE OR REPLACE VIEW api.stock_market_cap_daily AS SELECT s.symbol, d.trade_date AS date, d.total_market_cap FROM core.stock_market_cap_daily AS d JOIN ref.security AS s USING (security_id);
CREATE OR REPLACE VIEW api.index_daily AS SELECT d.trade_date AS date, i.index_code AS symbol, d.open, d.high, d.low, d.close, d.preclose, d.volume, d.amount, d.turn, d.close / d.preclose - 1.0 AS pct_chg, d.pe_ttm, d.ps_ttm, d.pcf_ncf_ttm, d.is_st FROM core.index_daily AS d JOIN ref.market_index AS i USING (index_id);
...
CREATE OR REPLACE VIEW api.stock_adjust_factor_coverage AS SELECT s.symbol, c.start_date AS start, c.end_date AS end FROM meta.stock_adjust_factor_coverage AS c JOIN ref.security AS s USING (security_id);
CREATE OR REPLACE VIEW api.stock_pb_daily_coverage AS SELECT s.symbol, c.start_date AS start, c.end_date AS end, c.field_set_id FROM meta.stock_pb_daily_coverage AS c JOIN ref.security AS s USING (security_id);
CREATE OR REPLACE VIEW api.stock_market_cap_daily_coverage AS SELECT s.symbol, c.start_date AS start, c.end_date AS end, c.field_set_id FROM meta.stock_market_cap_daily_coverage AS c JOIN ref.security AS s USING (security_id);
